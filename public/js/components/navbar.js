/**
 * Clean Neo-Brutalist Customer Navbar Component (SVG Icons)
 */
function initNavbar(activePage = 'home', user = null) {
    ...
    const navLinksHtml = navItems.map(item => `
        <li class="nav-item">
            <a class="nav-link px-3 py-2 fw-bold d-flex align-items-center gap-2 ${activePage === item.key ? 'active text-primary' : 'text-dark'}" href="${item.url}">
    ...
    if (user) {
        authSectionHtml = `
            <div class="d-flex align-items-center gap-3">
                <a href="/profil" class="d-flex align-items-center gap-2 text-dark text-decoration-none fw-bold px-3 py-1 bg-white border border-2 border-dark rounded-3 shadow-sm">
        ...
        authSectionHtml = `
            <div class="d-flex align-items-center gap-2">
                ...
                <a href="/register" class="btn btn-primary-custom btn-sm px-3 py-1">Daftar</a>
            </div>
        `;
    }

    navbarRoot.innerHTML = `
        <nav class="navbar navbar-expand-lg bg-white border-bottom border-2 border-dark sticky-top py-2">
            <div class="container">
